...
<script>
    export default {
        ...
        created(){
            ...
            this.axios.get(this.myurl+`admin/albums/editalbumsid`).then((res) => {
                this.edit_albums = res.data.albums
                this.edit_albums.forEach((v) => {
                    if (v.photos != '')
        ...
        methods: {
            handleAvatarSuccess(res, file) {
                this.edit_albums[this.index].image = res.url;
            },
            ...
            handleDelete(index) {
                this.edit_albums.splice(index, 1)

                let ids = this.edit_albums.map((v)=>{
                    return v.id
                });
                this.axios.post(this.myurl+`admin/albums/postalbumsid`,{ids:ids})
                if (this.edit_albums.length == 0)
            ...
            confirm_edit_pts() {
                ...
                this.edit_albums[this.index].photos = []
                this.fileList.forEach(v => {
                    this.edit_albums[this.index].photos.push(v.url)
                })
            },
            editIntrod(index) {
                this.index = index
                this.edit_Introd = this.edit_albums[index].introduction == ''
                this.textarea = this.edit_albums[index].introduction
                this.dialogIntrodVisible = true
            },
            confirm_edit_Introd() {
                this.edit_albums[this.index].introduction = this.textarea
            ...
            confirm_edit() {
                this.edit_albums.forEach(value => {
                    value.photos=value.photos.join("|")
                })
                this.axios.post(this.myurl+`admin/albums/edit`,this.edit_albums).then(()=>{
                    this.$router.push({name: 'albums'})
                })
            },
