    ...
    export default {
        ...
        methods: {
            ...
            handleAvatarSuccess(res, file) {
                this.new_singers[this.index].image = res.url;
            },
            ...
            handleDelete(index) {
                this.new_singers.splice(index, 1)


                if (this.new_singers.length == 0)
            ...
            confirm_add_pts() {
                ...
                this.new_singers[this.index].photos = []
                this.fileList.forEach(v => {
                    this.new_singers[this.index].photos.push(v.url)
                })
            },
            addIntrod(index) {
                this.index = index
                this.add_Introd = this.new_singers[index].introduction == ''
                this.textarea = this.new_singers[index].introduction
                this.dialogIntrodVisible = true
            },
            confirm_add_Introd() {
                this.new_singers[this.index].introduction = this.textarea
            ...
            confirm_add() {
                this.new_singers.forEach(value => {
                    value.photos=value.photos.join("|")
                })
                this.axios.post(this.myurl+`admin/singers/new`,this.new_singers).then(()=>{
                    this.$router.push({name: 'singers'})
                })
            },
